...
describe('External Notes API (e2e)', () => {
  ...
  it('POST /notes - create note', async () => {
    const response = await request(baseUrl)
      .post('/notes')
      .send({
        title: `Note ${uniqueTestValue}`,
        content: `Content ${uniqueTestValue}`,
      })
      .expect(201);
  ...
  it('POST /notes - create second note', async () => {
    const response = await request(baseUrl)
      .post('/notes')
      .send({
        title: `Second Note ${uniqueTestValue}`,
        content: `Second Content ${uniqueTestValue}`,
      })
      .expect(201);
    ...
    expect(response.body.content).toContain(
      `Second Content ${uniqueTestValue}`,
    );
  });
  ...
  it('GET /notes - retrieve list of notes (check count)', async () => {
    ...
    const titles = response.body.items.map(
      (note: { title: unknown }) => note.title,
    );
  ...
  it('GET /notes/:id - error 404 when retrieving nonexistent note', async () => {
    await request(baseUrl).get('/notes/nonexistent-id').expect(404);
  });
  ...
  it('DELETE /notes/:id - error 404 when deleting nonexistent note', async () => {
    await request(baseUrl).delete('/notes/nonexistent-id').expect(404);
  });
});
